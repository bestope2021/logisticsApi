 ...
 * 字段映射
 * Class FieldMap
 * @package smiler\logistics\Api\ShangMeng
 */
class FieldMap extends LsSdkFieldMapAbstract implements LsSdkFieldMapInterface
{
    ...
    public static function queryTrack(...$vars)
    {
        if ($vars[0] == LsSdkFieldMapAbstract::QUERY_TRACK_ONE) {
            $fieldKey = [
                'flag',// 处理状态： true 成功，false 失败
                'info',// 提示信息
                'ServiceNumber',// 查询单号可能是 客户订单号/第三方订单号|运单号/追踪号
                'ParcelStatus',// 订单状态
                'ParcelStatus',// 订单状态（货态）说明
                'TrackDetailsList',// 物流轨迹明细
            ];
        }

        if ($vars[0] == LsSdkFieldMapAbstract::QUERY_TRACK_TWO) {
            $fieldKey = [
                'CurrentPlace',// 订单状态（货态）
                'LatestInfo',// 订单状态（货态）描述
                'CurrentTime',// 订单状态（货态）时间
                'track_location',// 所在地
            ];
        }
    ...
    {
        $fieldKey = [
            'FreightWay_Code',// 运输方式代码
            'FreightWayName_EN',// 运输方式英文
            'FreightWayName_CN',// 运输方式中文
            'express_type',// 运输方式类型
