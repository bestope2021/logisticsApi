    ...
    public function createOrder(array $params = [])
    {
        ...
        foreach ($params as $item) {
            ...
            $ls[] = [
                ...
                'warehouse_code' => empty($warehousesmcode['warehouse_code']) ? 'USEA' : $warehousesmcode['warehouse_code'],
                'sm_code' => $sm_code ?? 'testAir',//transit_type=3特有。物流产品（整柜无需填）,我们后台设置的是快递
                'transit_warehouse_code' => empty($warehousesmcode['transit_warehouse_code']) ? '001' : $warehousesmcode['transit_warehouse_code'],
                ...
                'shiper_address' => [
                    ...
                    'sa_region' => $item['senderDistrict'] ?? '',//区
                    'sa_address2' => $item['senderAddress'] ?? '',//地址2
                ],
                'collecting_time' => date('Y-m-d H:i', time() + 2 * 60 * 60),//transit_type=3特有。揽收时间。注：揽收时间不允许小于创建时间,给他2小时后揽收
        ...
        $fieldData['id'] = $response['data']['receiving_code'] ?? '';
...
//        $this->dd($response, $ret, $reqRes);
    ...
    public function getShippingMethod()
    {
        ...
        //$fieldTypeMap=FieldMap::shippingMethodType();
        ...
        foreach ($res['data'] as $k => $v) {
            foreach ($v as $item) {
                //$item['type']=$fieldTypeMap[$item['type']];
