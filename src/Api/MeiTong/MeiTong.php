...
class MeiTong extends LogisticsAbstract implements BaseLogisticsInterface, PackageLabelLogisticsInterface, TrackLogisticsInterface
{
    ...
    public $interface = [
        ...
        'getShippingMethod' => 'get_transport', //获取配送方式

        'getServices' => 'get_services',//获取服务类型//服务类型。必填。有效值： all 所有服务；b2b B2B 大货服务；b2c B2C 小包服 务；ex 大货快递；
...
//        }
    ...
    public function request($function, $data = [])
    {
        ...
        switch ($this->req_data['key']) {
            ...
            case 'get_services':
                unset($data['key']);
                unset($this->req_data['key']);
                $response = $this->sendCurl('post', $this->config['url'] . $this->config['get_services_command'], $data, $this->dataType, $this->apiHeaders);
                break;//获取服务类型，有效值： all 所有服务；b2b B2B 大货服务；b2c B2C 小包服 务；ex 大货快递；
            default:
                unset($data['key']);
                unset($this->req_data['key']);
                $response = $this->sendCurl('post', $this->config['url'] . $this->config['create_order_command'], $data, $this->dataType, $this->apiHeaders);
    ...
    public function queryTrack($trackNumber)
    {
        ...
        $data = [
            'shipment' => ['client_reference' => $trackNumber],
            'validation' => ['access_token' => $this->config['access_token']],
            'key' => 'tracking',
        ];
        $response = $this->request(__FUNCTION__, $data);
